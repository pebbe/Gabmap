...
def setNumber():
    ...
    make = fp.read()
    fp.close()
    u.queue.enqueue(path + '/clu1det', make.format({'appdir': u.config.appdir, 'python3': u.config.python3, 'n': n}))
    for i in 'f1score.txt currentlist.txt currentselect.txt distmap.eps currentregex.txt'.split():
...
def setCluster():
    ...
    makes += '\t( for i in _/*.utxt; do echo `tail -n 1 $$i` $$i; done ) | grep -v ^_ | cdsort > f1score.txt\n'.format(c)
    makes += '\ttouch OK\n'
    u.queue.enqueue(path + '/clu1det', makes)
    u.queue.run()
    time.sleep(2)
...
u.path.chdir(username + '-' + path + '-clu1det')
...
sys.stdout.write('Location: goto?p={}-clu1det{}\n\n'.format(path, target))
