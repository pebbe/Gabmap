        ...
        if os.access('score.txt', os.F_OK):
            ...
            if curitem:
                ...
                if not os.access('distmap.eps', os.F_OK):
                    ...
                    fp.close()
                    os.chdir('..')
                    u.distribute.distmap(placen, placeall, 'clu2det/distmap', exfile='clu2det/distmap.ex', normalise=False)
                    os.chdir('clu2det')

            if curitem:
                sys.stdout.write(u.html.img('project_{}-clu2det-distmap'.format(pnum), True, usemap="map1", idx=2))
                ...
                if regex and not os.access('redistmap.eps', os.F_OK):
                    ...
                    fp.close()
                    os.chdir('..')
                    u.distribute.distmap(placen, placeall, 'clu2det/redistmap', exfile='clu2det/distmap.ex', normalise=False)
                    os.chdir('clu2det')

                if regex:
                    sys.stdout.write(u.html.img('project_{}-clu2det-redistmap'.format(pnum), True, usemap="map1", idx=3))
